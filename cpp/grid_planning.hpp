...
namespace mstar{
  ...
  /**
   * Same as find_grid path, but with EPEMstar
   *
   * @param increment how much to incrase the offset each planning iteration
   */
  std::vector<std::vector<std::pair<int, int>>> find_grid_epemstar_path(
    const std::vector<std::vector<bool>> &obstacles,
    const std::vector<std::pair<int, int>> &init_pos,
    const std::vector<std::pair<int, int>> &goals,
    double inflation, int time_limit, double increment);
}
