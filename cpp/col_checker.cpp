...
ColSet SimpleGraphColCheck::check_edge(const Coord &c1,
				       const Coord &c2,
				       const std::vector<int> ids) const{
  return simple_edge_check(c1.cbegin(), c1.cend(), c2.cbegin(), c2.cend());
}

ColSet SimpleGraphODColCheck::check_edge(const OdCoord &c1,
				       const OdCoord &c2,
				       const std::vector<int> ids) const{
  if (c2.is_standard()){
