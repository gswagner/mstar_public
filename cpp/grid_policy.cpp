...
EPEMstar_Policy mstar::epem_grid_policy(
    const std::vector<std::vector<bool>> &world_map,
    const std::pair<int, int> &goal){
  int columns = (int) world_map[0].size();
  return EPEMstar_Policy(get_graph(world_map, goal), goal.first * columns + goal.second);
}

EPEMstar_Policy* mstar::epem_grid_policy_ptr(
    const std::vector<std::vector<bool>> &world_map,
    const std::pair<int, int> &goal){
  int columns = (int) world_map[0].size();
  return new EPEMstar_Policy(get_graph(world_map, goal),
		    goal.first * columns + goal.second);
}
