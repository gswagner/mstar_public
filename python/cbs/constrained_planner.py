...
def find_constrained_path(obs_map, init_pos, goal, constraints,sub_search=None,
                          out_paths=None,conn_8=False, inflation=1.0,
                          time_limit=5*60):
    ...
    constraints - constraints defining where robot cannot go, should 
                    ...
                    robots
    out_paths   - paths of out-of group robots to try to avoid if possible,
    ...
    p = Constrained_Planner(obs_map, init_pos, goal, constraints,
                    sub_search=sub_search,conn_8=conn_8, inflation=inflation,
                            out_paths = out_paths)
...
class Constrained_Planner:
    ...
    a reviersed non-constrained plan, to provide heuristics for this planner
    
    set to provide resumable planning to different positions and times to
    provide policy style planning
    '''
    def __init__(self, obs_map, init_pos, goal, constraints,out_paths=None,
        ...
        constraints - constraints defining where robot cannot go, should 
                       consist of a single tuple, with only the appropriate 
                       robots
        out_paths   - paths of out of group robots to treat as soft constraints
        conn_8      - 8-connected grid if True; otherwise 4-connected
        only_init   - will only find paths to the original initial position
                      (pruning paths which cannot reach the specified position
                      in time).  If false, will not perform pruning'''
        ...
        # Graph is stored as dictionary indexed by strings of coord and tuple
        self.graph = {}
        # initialize with "normal" indices; is overwritten if other priority 
        #ordering is applied. Need time one greater than that of the last 
        #constraint to ensure happy 
        self.max_t = 0
        self.con_max_t = 0
        ...
        #configuration
        self.policy_key = self.rob_id+('backpolicy',)# +(tuple(init_pos),)#self.rob_id + 
        ...
        #Need persisitant open list for repeated planning, add the goal node
        #now, as it will be the initial search posotion
        ...
        self.open_list = SortedCollection.SortedCollection(
            [goal_node],key=lambda x:[-x.cost[0]-x.h*self.inflation,-x.cost[1]])
             #,-x.coord[-1]])
        #potentially need to depart the goal at multiple different times.  
        #Including all possible departure times will be a bit more efficient 
        #than forcing exploration
        prev = goal_node
    ...
    def heuristic(self, coord):
        '''Computes the heuristic costs for the robot from its position (coord)
        to the goal'''
    ...
    def get_transition_cost(self,old_coord,new_coord):
        '''Returns the increment in collision costs.  Note that this will
        return a partial tuple, that needs to be added elementwise to the old
        cost to get the new cost

        returns - (transition_cost, collision_count), where collision_count is
                  the number of collision with outpath robots
    ...
    def expand(self, node, open_list):
        '''Calls get_neighbor from workspace_graph, checks for collisions and 
        adds all found neighbors to the open_list used for performing 
        constrained planning'''
        #Need to both expand along at max_t, and prevent going beyond t=0
        ...
        for i in coords:
            ...
            if (new_cost < new_node.cost):
                #Check for constraint violation, stepping from i to node.
                #Remember, that i is temporally before node
                if cbs.con_is_constrained(self.constraints,node.coord[-1],i[:2],
                                      node.coord[:2]):
    ...
    def find_path(self, start_pos,time_limit=5*60):
        ...
        position all internal computation is done in an explicitly single 
        robot space.  Actually plans from goal to init pos
        ...
        while len(self.open_list) != 0:
            ...
            if node.closed:
                #Don't remove previous references to node from the open list,
                #instead just skip duplicates stored in more expensive
                #places
                continue
            node.open = False
            node.closed = True
            self.expand(node, self.open_list)
            # robot has reached goal and other robots are already at goal 
            #position.  Have to terminate search after expansion, so that we
            #don't lose this nodes neighbors for later search
    ...
    def get_cost(self,coord):
        '''Finds the cost of moving from a specified configuration to the goal,
        used for generating more informed heuristics
        coord - (x,y,t) '''
        coord = tuple(coord)
        end_node = self.get_node(coord)
        if end_node.closed:
            #Have already found a path from the specified configuration to the
            #goal, so can go ahead and return the cost
            return end_node.cost[0]
        #Need to extend our search to connect to the specified configuration
        path,cost = self.find_path(coord)
        return cost

    def get_h_cost(self,coord):
        '''Finds the cost of moving from a specified configuration to the goal,
        including both cost to go and expected collisions
        coord - (x,y,t) '''
        coord = tuple(coord)
        end_node = self.get_node(coord)
        if end_node.closed:
    ...
    def get_neighbors(self,coord):
        '''Returns the collision free neighbors of the specified coordinates
        coord - (x,y,t)'''
        coords = self.sub_search[self.policy_key].get_forwards_neighbors(coord,
                                                                    self.max_t)
        return [c for c in coords if not cbs.con_is_constrained(
                self.constraints,c[-1],coord[:2],c[:2])]

    def get_step(self,coord):
        '''Gets the policy for the given coordinates, extending planning to
        reach if necessary
        coord - (x,y,t)'''
        coord = tuple(coord)
    ...
    def gen_limited_offset_neighbors(self,node):
        '''Stores the neighbors of a node in terms of changes in f-value, i.e.
        sum of cost to reach the node and the cost to go.  Only considering the
        direct f-value cost, does not use the collision count, as finding the 
        proper time to transition from incrementing direct cost to collision
        count would be hard
    ...
    def get_offset_neighbors(self,coord,offset):
        '''Returns all neighbors of coord with the specified EPEA* offset
        (i.e. difference in f-value).  This is with respect to some multirobot
        planner, so the f-value is capturing the true cost to go at the single 
        robot level
    ...
    def get_limited_offset_neighbors(self,coord,max_offset,min_offset=0):
        '''Returns the set of neighbors for which the maximum difference in
        path cost if passed through is less than the specified value.  (i.e.
        if you are forced to pass through coordinate x, instead of the optimal
        step, what is the difference in cost?)  Does not include the count of
        outpath collisions in this calculation

        coord - coordinates of the node to find neihbors of
        max_offset - the maximum increase in path cost to encur in choice of
                     neibhors
    ...
    def get_offsets(self,coord):
        '''Return the possible offsetes (i.e. differences in f-value) of the
        neighbors
...
class Constrained_Forwards_Planner:
    ...
    def __init__(self, obs_map, init_pos, goal, constraints,sub_search=None, 
        ...
        constraints - constraints defining where robot cannot go, should 
                       ...
                       robots
        conn_8      - 8-connected grid if True; otherwise 4-connected
        out_paths   - paths of other robots to avoid, if possible without 
                      incuring extra cost'''
        self.obs_map = obs_map
        ...
        if self.sub_search == None:
            self.sub_search = {self.rob_id:workspace_graph.Astar_Graph(obs_map,
                                                            goal, self.conn_8)}
    ...
    def expand(self, node, open_list):
        ...
        for i in coords:
            ...
            # step_cost = 1
            # # cost = 0 when robot stays at goal position
            # if node.coord == self.goal and new_node.coord == self.goal:
            #     step_cost = 0
    ...
    def transition_cost(self,prev_node,new_node):
        '''Computes the cost of the new neighbor when steping from prev_node
        ...
        out_col = self.col_check.single_bot_outpath_check(new_node.coord,
                                                          prev_node.coord,
                                               new_node.t,self.out_paths)
        return [prev_node.cost[0]+step_cost,prev_node.cost[1]+out_col]
        # return prev_node.cost+step_cost
    ...
    def find_path(self, time_limit=5*60):
        ...
        # open_list = SortedCollection.SortedCollection([first], 
        #                     key=lambda x:-x.cost-x.h*self.inflation)
        open_list = SortedCollection.SortedCollection([first], 
                            key=lambda x:[-x.cost[0]-x.h*self.inflation,
                                           -x.cost[1]])
...
def find_forwards_constrained_path(obs_map, init_pos, goal, constraints,
                                   sub_search=None,conn_8=False, inflation=1.0,
                                   time_limit=5*60,out_paths = None):
        '''initialize variables (obstacle map, positions, ...).  
        obs_map     - binary obstacle map
        init_pos    - start positions
        goal        - goal position 
        constraints - constraints defining where robot cannot go, should 
                       ...
                       robots
        conn_8      - 8-connected grid if True; otherwise 4-connected'''
        p = Constrained_Forwards_Planner(obs_map, init_pos, goal, constraints,
                      sub_search=sub_search,conn_8=conn_8, inflation=inflation,
                                         out_paths = out_paths)
        #Need to specify when you are starting your path from
...
class ID_Constrained_Planner(Constrained_Planner):
    '''Variant to interface with the Independence detection code.  Only
    takes in paths, and determines whether they must be converted to 
    cbs constraints (for hard constraints), or can be treated as a simple
    path with no constraints (soft constraints/tie breaking)

    Note: Hard constraints will produce a path at least 1 step longer than 
    the seed path'''
    def __init__(self,obs_map,init_pos,goal,hard_constraint,out_paths,rob_id,
                 sub_search=None, conn_8=False,inflation=1,):
        '''obs_map     - binary obstacle map
        init_pos    - (x,y,t) start position and time
        goal        - goal position 
        hard_constraint - whether out_paths should be treated as a hard 
                          constraint
        out_paths   - paths of out of group robots to treat as soft constraints
                      Set None to ignore
        conn_8      - 8-connected grid if True; otherwise 4-connected'''
        if (not hard_constraint) or out_paths == None:
            #Can feed in empty constraints and the normal path for tie breaking
            Constrained_Planner.__init__(self,obs_map,init_pos,goal,
                                         cbs.con_empty_constraint((rob_id,)),
                                         out_paths=out_paths,
                                         sub_search=sub_search,conn_8=conn_8,
                                         inflation=inflation)
            return
        test_time = timer.time()
        #Need to start by converting the paths into hard constraints
        con = cbs.con_empty_constraint(rob_id)
        for t in xrange(len(out_paths)):
            for bot_pos in out_paths[t]:
                con = cbs.con_add_node_constraint(con,t,bot_pos)
        #Need to add the edge constraints
        for t in xrange(1,len(out_paths)):
            for bot_dex in xrange(len(out_paths[t])):
                con = cbs.con_add_edge_constraint(con,t,
                                                   out_paths[t-1][bot_dex],
                                                   out_paths[t][bot_dex])
        Constrained_Planner.__init__(self,obs_map,init_pos,goal,
                                     con,out_paths=None,
                                     sub_search=sub_search,conn_8=conn_8,
                                     inflation=inflation)
        #After max_t has been computed, need to go and add the final
        #positions of the robots as constraints at max_t, so cannot plow
        #through the final location of the out-group robots
        for bot_pos in out_paths[-1]:
            con = cbs.con_add_node_constraint(con,self.max_t,bot_pos)
